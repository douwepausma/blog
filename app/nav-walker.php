 ...
 * Class NavWalker
 * 
 * Custom walker with cleaner markup for wp_nav_menu()
...
class NavWalker extends \Walker_Nav_Menu {
  ...
  function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
    ...
    $item_color = get_field('category_color', get_term($item->object_id));

    // Class list for the link node
    $item_classes = ['nav-link badge'];
    
    // Add active / badge class with corresponding category color when the menu item is the current one
    if($item->current || $item->current_item_parent) {
      array_push($item_classes, 'badge-'. $item_color);
    }

    parent::start_el( $item_html, $item, $depth, $args );
    
    if ( $item->is_subitem ) {
      array_push($item_classes, 'dropdown-toggle');
      $item_html = str_replace( '<a', '<a class="'. implode($item_classes, ' ') .'"', $item_html );
      $item_html = str_replace( '</a>', ' <i class="fas fa-chevron-down fa-fw text-primary"></i></a>', $item_html );
    } else {
      $item_html = str_replace( '<a', '<a class="'. implode($item_classes, ' ') .'"', $item_html );
    }
